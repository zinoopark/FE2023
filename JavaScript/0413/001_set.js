...
for (const i of 게시자) {
  // 중복이 허용되지 않는, 즉 이호준, 이상용, 김연하만 순회함.
  console.log(
    i,
    회사게시판.filter((e) => e === i)
    // 전체 회사게시판 배열중, 각각의 i(이호준, 이상용, 김연하와 공통으로 일치하는 요소들을 배열화)
    // 이호준 (5) ['이호준', '이호준', '이호준', '이호준', '이호준']
    // 이상용 ['이상용']
    // 김연하 ['김연하']
  );
}
...
for (const i of 회사게시판) {
  //회사게시판이 배열이기 때문에 i의 첫순서는 0
  map.set(i, (map.get(i) || 0) + 1); //맵에 뭔가 있으면(get)벨류 넣어주고, 없으면 0, ||는 둘다 false여야함
}
...
// 차집합
let dif = [...a].filter((e) => !b.has(e)); //b의 요소들을 모두(공통된 것 까지)제외 한 순수 a
